...
if [ "$USE_NGROK" = true ]; then
    if ! command -v ngrok &> /dev/null; then
        ...
        if [ -n "$NGROK_URL" ]; then
            ...
            echo "  ----------------------------------------"
            echo ""
            echo "  To add as a red team target in Strata Cloud Manager:"
            echo "  AI Security > AI Red Teaming > Targets > + New Target"
            echo ""
            echo "  Import this cURL:"
            echo "    curl -X POST $NGROK_URL/api/v1/chat/completions \\"
            echo "      -H 'Content-Type: application/json' \\"
            echo "      -d '{\"messages\":[{\"role\":\"user\",\"content\":\"{INPUT}\"}],\"max_tokens\":1024}'"
        else
            echo ""
